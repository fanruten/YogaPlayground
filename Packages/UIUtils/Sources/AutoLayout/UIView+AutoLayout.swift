...
extension AutoLayoutItem {
    ...
    @discardableResult
    public func pinSize(width: CGFloat, height: CGFloat) -> [NSLayoutConstraint] {
        return [pin(.width).const(width).equal(),
                pin(.height).const(height).equal()]
    }

    @discardableResult
    public func pinSize(square: CGFloat) -> [NSLayoutConstraint] {
        return pinSize(width: square, height: square)
    }
}

extension UIView {
    public func pinToSuperView(_ attribute: NSLayoutConstraint.Attribute) -> LayoutConstraintBuilder {
        return pin(attribute).to(superview!)
    }

    public func pinEdgesToSuperView(edges: UIEdgeInsets = .zero) {
