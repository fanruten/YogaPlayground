...
///  Provides the method needed to compare the identity of two objects.
...
/// Same algorithm you can find in IGListKit (https://github.com/Instagram/IGListKit/blob/master/Source/Common/IGListDiff.mm).
public enum List {
    ...
    /// https://github.com/Instagram/IGListKit/blob/master/Source/Common/IGListIndexPathResult.h
    public struct Result {
        ...
        public var updatesNew = IndexSet()

        /// The itemId-to-index map in the old collection.
        public var oldMap = Dictionary<AnyHashable, Int>()

        /// The itemId-to-index map in the new collection.
        public var newMap = Dictionary<AnyHashable, Int>()

        /// Indicates whether the result has any changes or not.
        public var hasChanges: Bool {
            return (self.inserts.count > 0) || (self.deletes.count > 0) || (self.updatesNew.count > 0) || (self.moves.count > 0)
        }

        /// The number of changes in the new collection (inserts + deletes + updatesNew + moves).
        public var changeCount: Int {
            return self.inserts.count + self.deletes.count + self.updatesNew.count + self.moves.count
        }

        // Checks that number of items in the new collection correspond to number of items in the old collection with considering of inserts/deletes.
        ...
        /// The index of item with `identifier` in old collection.
        public func oldIndexFor(identifier: AnyHashable) -> Int? {
            return self.oldMap[identifier]
        }

        /// The index of item with `identifier` in new collection.
        ...
        /* let insertOffsets */_ = newRecords.enumerated().map { (i, newRecord) -> Int in
            ...
            if let oldIndex = newRecord.index {
                ...
                if newRecord.entry.updated {
                    result.updatesOld.insert(oldIndex)
                    result.updatesNew.insert(i)
                }
